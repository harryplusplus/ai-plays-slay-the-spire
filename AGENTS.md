...
- **`sts-v2` bank** (현재 사용 중): ~192 memory units, 102개 documents
  - EXPERIENCE 84개, OBSERVATION 79개, WORLD 29개
...
- **END 후 retain 누락 패턴**: AI가 자발적으로 retain tool을 호출하지 않는 경우 ~30%
  - 원인: System prompt로 유도하지만, tool call은 LLM의 선택
  - 근본 해결책: `main.py`에서 END 후 code-level 강제 retain 호출 (미구현)
