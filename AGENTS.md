...
2. **학습 가능한 AI** — 각 런에서 배운 전략, 패턴, 교훈을 Hindsight 장기기억에 쌓아서 다음 런에 활용
3. **지속적 개선** — retain/recall 사이클로 시간이 지날수록 플레이 품질 향상

## 문서화 메타: 이 파일과 README를 어떻게 다룰까

### 이 문서(AGENTS.md)의 역할
- **실행 레퍼런스 + 현재 상황판**: 뭘 재시작해야 하는지, 현재 무슨 문제가 있는지, 다음에 뭘 할지 확인하는 기준
- **협업 공유 문서**: Harry랑 AI 에이전트가 둘 다 읽고 참고하면서, 작업 결과를 바탕으로 같이 업데이트하는 살아있는 문서
- **상황 인식 도구**: 프로젝트 현재 상태, 시도한 것, 배운 것, 다음 단계를 기록해서 의사결정 일관성 유지

### README.md의 역할
- **프로젝트 얼굴**: 외부(또는 미래의 나)가 볼 때 이 프로젝트가 뭔지, 왜 특별한지, 어떻게 협업하는지
- **메타인지 기록**: 기술 설명서가 아니라 "우리가 뭘 하면서 어떻게 배워가는가"에 대한 기록
- **협업 철학**: Harry랑 AI 에이전트의 역할 분담, 의사결정 방식

### 문서화 개선 역사
1. **초기**: 간단한 AI 에이전트 노트 (기능 목록 중심)
2. **코드 기준 사실 바로잡기**: README랑 AGENTS.md가 실제 코드랑 달랐음 → 코드 직접 읽고 구조/데이터 흐름 정확히 기술
3. **메타인지 전환**: 기술 설명서 → "우리가 뭘 하면서 뭘 배우는가" 기록
4. **역할 분담 명시**: "Harry는 뭘 하고 AI는 뭘 하는가" 명확히 기술
5. **현재 상황 중심 재구조화**: "문제 → 해결 → 다음" 흐름으로 정리

### 문서화 원칙 (앞으로 지킬 것)
- **코드와 문서 불일치 시 코드가 우선**: 문서가 코드보다 앞서가지 않도록. 코드 변경 후 문서 바로 업데이트
...
- **AGENTS.md는 한국어 반말**: Harry랑 대화하듯이 써. 토큰 효율도 좋고.
- **README도 한국어 반말 + Harry 명시**: "Harry" 이름 명확히 사용, AI 에이전트가 작성 중임을 고지
...
- `sts` bank의 저품질 메모리(상태 스냅샷)는 빼고, 2026-04-24 09:00 UTC 이후 고품질 메모리 101개를 `sts-v2`로 옮겼어
- `sts-v2` 현재 상태: ~285 memory units, EXPERIENCE/OBSERVATION 중심

#### retain_async=True로 타임아웃 해결
- Python SDK `retain()` 기본값이 `retain_async=False`라서 서버가 작업 완료까지 blocking
- `retain_batch(retain_async=True)`로 바꿔서 즉시 반환, 백그라운드 worker 처리
...
#### LLM 메시지 덤프
- `~/.sts/logs/llm_dump/llm_dump_{timestamp}.json`에 LLM API 호출 직전 메시지 배열 저장
- 행위 기반 로테이션: 최근 **10개** 유지
- 각 덤프는 system + user + assistant + tool 체인 전체 포함
- LLM이 실제로 보는 컨텍스트를 정확히 분석 가능

#### LLM 응답 시간 측정
- `ai.jsonl`의 `llm_response` 이벤트에 `duration_ms` 필드 추가
- `jq 'select(.event == "llm_response") | {ts, duration_ms}'`로 분석 가능

#### Recall 쿼리 개선
- `class` 추가: `IRONCLAD room=MonsterRoomElite act=1 screen=NONE monsters=Sentry`
...
- **END 후 retain 누락 패턴**: **실제 누락 0건** — 이전 분석에서 "END 후 다음 커맨드까지 retain 없음"을 누락으로 봤는데, 그 사이에 전투가 계속된 경우(다음 턴 PLAY/END/WAIT)도 포함됐던 거였어. 전투 종료 후 COMBAT_REWARD/MAP 도달 시에는 모두 retain 있음.

#### Recall
- **정상 작동**하는데 응답 지연이 심해: **~26초** (embedding 5s + reranking 21s)
- 원인: OpenClaw bank의 batch_retain 작업이랑 Hindsight worker 리소스 경쟁
- 현재 116 facts / 4059 tokens 반환 — context 과다 가능성
- 기능적으로는 문제없는데 전체 턴 시간이 늘어나
...
처음엔 프롬프트가 "매 커맨드 후 retain"을 강제해서 한 턴에 4번 retain이 쌓였어. 내용은 대부분 "에너지 2→1" 같은 순간 상태 스냅샷. Hindsight fact extraction이 EXPERIENCE/OBSERVATION으로 분해 저장했는데, 원본 fact 자체가 의미없어서 Observation도 의미없는 noise만 쌓였지.
...
- Observation은 이전 기억의 강도를 보정해주는데, **원본 fact 자체가 의미있어야** 의미있는 Observation이 생겨
- **Context field는 high-impact** — extraction 방향을 결정
- **Retain과 recall을 같은 턴에 하지 말 것** — retain은 write, recall은 read. 쓴 건 다음 턴에야 검색 가능
...
6. **Entity labels 도입** — `enemy_name`, `act`, `card_type` 등으로 정형화
7. **Mental model 생성** — 직업별 빌드 가이드, Act별 적 대응 전략 등 reflect 결과를 mental model로 고정
8. **Recall 쿼리 추가 실험** — 자연어 vs 키워드, `max_tokens` 조정, `budget=high` 등

### 장기 (목표 달성 과정)
9. **Reflect 활용** — 단순 recall이 아닌 Hindsight reflect로 전략 조언 받기
10. **Budget 조정** — `recall_budget_function`을 `adaptive`로 변경해 검색 깊이 동적 조절
11. **심장 클리어**
...
Hindsight는 `/Users/harry/repo/nailed-it/external/hindsight/`에 있고, 이 프로젝트랑 별도 저장소야. 우리는 Hindsight 소스코드를 직접 읽고 분석할 수 있어.
...
**결과**: `hindsight memory recall` 기본 호출 시 `observation` 타입 메모리가 검색 결과에서 **제외됨**. 우리가 개선한 고품질 기억이 consolidate되어 `observation`으로 생성되었는데 recall로 검색이 안 되는 치명적 불일치였어.

**해결**: `game/cli.py`에서 `--fact-type world --fact-type experience --fact-type observation`을 명시적으로 전달해서 CLI의 구식 기본값을 우회했어.

이게 우리가 소스코드를 직접 읽어야만 발견할 수 있었던 버그야.
...
- **메시지 트리밍**: 총 메시지 문자 수가 **1MB** 초과 시 오래된 완전 턴(assistant+tool+user)을 드롭. 시스템 메시지는 보존. (deepseek-v4-flash의 1M 토큰 컨텍스트에 맞춰 400KB→1MB로 증가)
