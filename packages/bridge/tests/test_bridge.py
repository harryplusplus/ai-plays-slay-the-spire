...
@pytest.mark.asyncio
async def test_process_next_iteration_processes_pending_command_without_message(
    tmp_path: Path,
) -> None:
...
async def test_process_next_pending_command_returns_false_without_pending_command(
    ...
    sqlite_file = tmp_path / "no-pending-command.sqlite"
    engine = db.create_engine(sqlite_file)
    sessionmaker = db.create_sessionmaker(engine)
    output_stream = io.StringIO()
...
@pytest.mark.asyncio
async def test_process_next_iteration_sleeps_when_message_and_pending_are_absent(
    tmp_path: Path,
) -> None:
