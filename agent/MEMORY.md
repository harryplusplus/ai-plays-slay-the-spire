...
- `./scripts/sts-summary.sh`
  - 최신 상태에서 전투 요약을 짧게 뽑고, 비전투 선택 화면이면 선택 요약도 함께 뽑습니다.
  - 전투에서는 손패, 에너지, 적 의도, 오브, 파워, discard의 0코스트 카드가 포함됩니다.
  - 비전투에서는 `choice_list`, `keys`, `screen.options`, `next_nodes`, `GRID` 의 `confirm_up` 같은 정보가 포함됩니다.
  - 현재 작업 디렉터리에서 그대로 실행합니다. 실행 전에 디렉터리를 바꾸지 않습니다.

## Usage Notes
- 두 스크립트 모두 미래 메시지를 기다리지 않고, 이미 기록된 이벤트만 읽습니다.
- 최근 이벤트 범위 안에 `message`가 없으면 실패하므로, 그때는 `50` 같은 더 큰 limit로 다시 실행합니다.
- 스크립트와 이 메모리는 플레이 흐름, 최신 mod 동작, 반복 작업 패턴에 맞춰 필요할 때 주기적으로 업데이트합니다.
...
- [x] `./scripts/sts-summary.sh` 가 그대로 동작하는지 확인합니다.
...
### Script Notes
- `./scripts/sts-summary.sh` 는 최신 mod 기준 비전투 선택 정보까지 요약할 수 있도록 확장했습니다.
- 이전 이름 `./scripts/sts-combat-summary.sh` 는 제거했습니다. 현재는 `./scripts/sts-summary.sh` 만 사용합니다.
